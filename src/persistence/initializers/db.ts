...
export class DbDataInitializer implements IDataInitializer {
    ...
    async initData(): Promise<number> {
        ...
        const ratings: IRating[] = [
            ...
            { userId: '3', guideId: (await this.unitOfWork.guides.getGuidesByName('A Guide with name 2'))[0].id, rating: 5 },
            { userId: '1', guideId: (await this.unitOfWork.guides.getGuidesByName('Callcenter access 3000'))[0].id, rating: 1 },
            { userId: '2', guideId: (await this.unitOfWork.guides.getGuidesByName('History of Linz'))[0].id, rating: 3 },
            { userId: '3', guideId: (await this.unitOfWork.guides.getGuidesByName('History of Linz'))[0].id, rating: 2 },
            { userId: '1', guideId: (await this.unitOfWork.guides.getGuidesByName('Callcenter access 3000'))[0].id, rating: 1 },
            { userId: '2', guideId: (await this.unitOfWork.guides.getGuidesByName('History of Linz'))[0].id, rating: 1 },
            { userId: '3', guideId: (await this.unitOfWork.guides.getGuidesByName('Callcenter access 3000'))[0].id, rating: 5 },
            { userId: '1', guideId: (await this.unitOfWork.guides.getGuidesByName('A man must do, what a man has to do'))[0].id, rating: 4 },
            { userId: '2', guideId: (await this.unitOfWork.guides.getGuidesByName('History of Linz'))[0].id, rating: 3 },
            ...
            { userId: '2', guideId: (await this.unitOfWork.guides.getGuidesByName('History of Linz'))[0].id, rating: 5 },
            { userId: '3', guideId: (await this.unitOfWork.guides.getGuidesByName('Callcenter access 3000'))[0].id, rating: 4 },
            { userId: '1', guideId: (await this.unitOfWork.guides.getGuidesByName('A man must do, what a man has to do'))[0].id, rating: 4 },
            { userId: '2', guideId: (await this.unitOfWork.guides.getGuidesByName('Callcenter access 3000'))[0].id, rating: 1 }
        ];
